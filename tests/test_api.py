...
class Harness:
    ...
    def build_worker(self, *, run_loop: bool = True) -> ApplicationWorker:
        ...
        `run_loop=False` is what a test driving `drain()` by hand needs: a
        running loop is draining the same queue, so the two would race for
        the claims — which is also why `drain()` is only ever called by the
        loop, or by a caller that asked for no loop.
        """
...
def staged_application(harness: Harness, queue_id: int) -> int:
    ...
    decision sent in between is correctly refused as in progress, so a test
    that waited only for the row would be racing its own worker.
    """
