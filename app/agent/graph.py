...
def build_graph(
    ...
    async def attribute_fields(state: ApplicationState) -> dict[str, Any]:
        ...
        if result is None:  # pragma: no cover - trigger_autofill always sets it
            raise PageUnavailable(state["thread_id"])
        attributed: list[str] = []
    ...
    async def fill_gaps(state: ApplicationState) -> dict[str, Any]:
        ...
        cost = plan.model_cost
        deps.db.update_application(state["application_id"], model_cost=float(cost))
        return {
            "model_cost": str(cost),
            "gap_summary": [dict(entry) for entry in deps.gap_filler.log_payload(plan)],
            "blocking_reasons": _blocking(plan, unwritten, state.get("settled", True)),
        }
    ...
    async def submit(state: ApplicationState) -> dict[str, Any]:
        ...
        if not outcome.submitted:
            return await _terminal(
                ...
                "submit",
                RunStatus.FAILED,
                outcome.reason or "the submission did not complete",
                ApplicationStatus.FAILED,
                QueueState.FAILED,
...
class ApplicationRunner:
    ...
    async def resume_application(
        ...
        another application's audit row, let alone releases it. Only then is
        the checkpoint consulted: an identical decision for a thread that
        already resumed replays (returning the outcome it produced), a
        different one conflicts, and a thread that never reached the gate is
        refused.
        """
